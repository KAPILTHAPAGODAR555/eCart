...
function ShowPage() {
    ...
    const handleReview = async(e)=> {
        ...
        let result = unwrapResult(await dispatch(addReview({id: id , review: review})));
    ...
    const handleDeleteReview = async(reviewId)=> {
        ...
        let result = unwrapResult(await dispatch(deleteReview({id: id , reviewId: reviewId})))
        let {message , success} = result;
    ...
    const handleCart = async() => {
        try {
            await dispatch(AddToCart({id: id}));
        } catch (error) {
                handleError(error);
        }
    }
                    ...
                    {info?.reviews?.map((element, index)=> {
                        ...
                        return(
                                ...
                                <div className={`col-12 col-md-4 col-lg-5 border rounded ${isPhone ? 'ms-1 m-3' :'ms-4'} mt-2 p-2`}>
                                   ...
                                   {userId != null && element.author[0]._id === userId &&  <span className='badge text-bg-light pt-2 text-danger fs-6 del-rev' style={{cursor: 'pointer'}} onClick={()=> handleDeleteReview(element._id)}>Delete</span>}
                                    </div>
                                </div>   
